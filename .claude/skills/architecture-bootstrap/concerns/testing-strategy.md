...
ADR `testing-strategy` con: proporciones de la pirámide objetivo (unit / integración / e2e), política de mocks vs fakes vs reales con criterio claro (qué se mockea y qué no y por qué), si TDD es obligatorio o recomendado y para qué tipo de código, y el umbral de cobertura si se acordó (sobre qué capas y con qué métrica). El criterio de mocks escrito como regla verificable: "nunca mockear clases internas del dominio; siempre mockear llamadas HTTP salientes y acceso a DB en unit tests".

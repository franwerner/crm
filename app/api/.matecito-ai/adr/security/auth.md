...
- Passwords hasheados con `Bun.password` (nunca en texto plano, nunca logueados — ver `../runtime/error-handling.md` §4.5).
- Tokens firmados/verificados con `hono/jwt`. El secret de JWT viene del módulo de config validado (`../delivery/configuration.md`).
...
- Atributos de la cookie (nombre, dominio, max-age, `Secure` por entorno) salen del módulo de config validado (`../delivery/configuration.md`), no hardcodeados.
- CORS configurado para el mismo sitio con `credentials` habilitado; orígenes permitidos vienen de config (`../delivery/configuration.md`). No usar `*` con credentials.

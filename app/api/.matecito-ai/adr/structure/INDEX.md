...
| [cross-slice-id-resolution.md](cross-slice-id-resolution.md) | Accepted | Resolución de IDs cross-slice por filtro | Un slice (ej. `enrichment`) necesite resolver IDs de otro (`contacts`) aplicando filtros vía read-port sobre `@shared/db`; reuses la gramática de filtros entre módulos; registres un nuevo consumo cross-slice de una tabla de otro módulo. |

...
└── tests/                                    # ver `../delivery/testing-strategy.md` (Pending)
```
...
> **Nota (ver `../contracts/api-contract.md`):** la capa presentation del slice vive bajo `http/`: `<entity>.routes.ts` (OpenAPIHono + createRoute + registro), `<entity>.controller.ts` (solo funciones handler), `dto/in/` y `dto/out/` (zod + `z.infer` del borde). El estilo está condicionado por la decisión de documentación/contrato de API — consultá `../contracts/api-contract.md` antes de crear o tocar rutas.

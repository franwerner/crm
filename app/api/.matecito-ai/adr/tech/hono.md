...
- **Implicancia arquitectónica clave:** Hono NO inyecta dependencias. La inyección de dependencias y el composition root son responsabilidad nuestra — ver `../structure/architecture-style.md`, `../structure/layers-and-dependencies.md` y `../delivery/dependency-injection.md`.
- API REST sobre Hono: usar `hono/validator` o adapter (zod) para validación en el borde — ver `../structure/inter-layer-communication.md`.
- Manejo de errores: Hono expone `app.onError` para un handler global — ver `../runtime/error-handling.md`.

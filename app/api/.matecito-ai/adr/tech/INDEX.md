...
| Tech | Versión | Por qué |
|---|---|---|
| [minio](minio.md) | latest (image tag, sin pinear — decisión consciente para dev) | S3-compatible auto-hosteado para dev; mismo cliente `Bun.s3` nativo funciona contra MinIO o AWS S3 cambiando endpoint. `../data/file-storage.md` |

### Validación
| Tech | Versión | Por qué |
|---|---|---|
| [zod](zod.md) | latest / sin pinear | Validación de forma/tipos en el borde + inferencia TS; adapter oficial de Hono. Reusado en config (`../delivery/configuration.md`) |
...
| Tech | Versión | Por qué |
|---|---|---|
| [zod](zod.md) | (ver Validación) | Schema de config validado al startup en `src/shared/config` (`../delivery/configuration.md`) |

### Auth
_Sin dependencia externa — se usan `hono/jwt` (parte de Hono) y `Bun.password` (nativo de Bun). Ver `../security/auth.md`. Refresh/rotación: Pending._
...
| Tech | Versión | Por qué |
|---|---|---|
| [hono-zod-openapi](hono-zod-openapi.md) | latest / sin pinear | Schema zod del borde = validación + OpenAPI 3.x autogenerado (fuente única de verdad). `../contracts/api-contract.md` |
| [scalar-hono-api-reference](scalar-hono-api-reference.md) | latest / sin pinear | UI de docs moderna para humanos sobre el mismo spec. `../contracts/api-contract.md` |

> **kubb** NO se registra acá: es consumidor del OpenAPI y vive en `app/ui` (se registra en su catálogo cuando se bootstrapee ese paquete). Ver `../contracts/api-contract.md`.

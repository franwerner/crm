# ADR — Ordenamiento server-side en endpoints de listado
...
El parseo de `sort` ocurre en la capa Zod (`buildListQuerySchema`), antes de llegar al repositorio. Errores lanzan `ValidationError` (400, `code: validation_failed`), coherente con `../runtime/error-handling.md`.

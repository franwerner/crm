...
- **Naming de la capa DB:** tablas y columnas en `snake_case`. Claves foráneas con sufijo `_id` (ej: `contact_id`, `author_id`, `created_by`). El naming del código TypeScript sigue `../structure/folder-structure.md`; esta regla aplica solo a la definición del schema Drizzle.

...
- **Version:** image tag `minio/minio:latest` (sin pinear — decisión consciente para dev, ver Notas)
- **Status:** Accepted
- **Decided in phase:** file-storage
- **Date:** 2026-05-26
...
- **Cliente: `Bun.s3` NATIVO** (`S3Client` integrado en Bun ≥ 1.2). NO requiere `@aws-sdk/client-s3` ni otro SDK npm. Se adopta como cliente único contra cualquier endpoint S3-compatible.
- Endpoint configurado vía env var `MINIO_ENDPOINT` (ver `../delivery/configuration.md` + `src/shared/config`).
- Convenciones de buckets, naming de keys, límites de tamaño y MIME whitelist: ver `../data/file-storage.md`; el cliente `Bun.s3` lo encapsula la capability compartida `src/shared/storage`.

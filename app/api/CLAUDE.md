...
Las decisiones, convenciones y políticas de ingeniería de este paquete (`app/api`) están en `.matecito-ai/edr/` (EDR — Engineering Decision Records).

**Antes de escribir código que toque arquitectura, capas, errores, auth, datos o convenciones, leé `.matecito-ai/edr/INDEX.md`** para saber qué EDR consultar.

**Antes de instalar/sugerir cualquier dependencia nueva (lib, framework, herramienta, DB), leé `.matecito-ai/edr/tech/INDEX.md`** para ver qué tecnologías ya están elegidas. Si tu sugerencia pisa con algo ya registrado, no la introduzcas sin preguntar al usuario.

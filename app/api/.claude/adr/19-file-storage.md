# ADR 19 — Storage de archivos (object storage)
...
El módulo `projects` (y previsiblemente otros futuros: contratos, comprobantes, adjuntos genéricos) necesita almacenar archivos binarios asociados a entidades del dominio. La base de datos relacional (ADR 09) no es el lugar correcto para blobs: degrada backups, escala mal y mezcla responsabilidades.
...
- se acceda solo desde la capa de infraestructura (mismo principio de encapsulación que los repositorios — ADR 02 #4),
...
### Encapsulación (regla #4 del ADR 02 aplicada a storage)
...
- Cada slice que necesite storage declara su puerto `*.storage.ts` en `domain/` (interface) y su adapter `*.storage.bun.ts` en `infrastructure/`. Análogo al patrón `*.repository.ts` / `*.repository.bun.ts` del ADR 09.
...
- `module` y `entity` en plural minúsculas (alineado con tablas DB — ADR 13).
...
Cualquier otro tipo se rechaza con `ValidationError` (400 — ADR 04). La lista se amplía solo via update a este ADR.
...
- Cero dependencias npm para storage — alineado con el patrón del repo (Bun-nativo, igual que `hono/jwt` y `Bun.password` en ADR 10).
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-26 | Decisión inicial. MinIO + Bun.s3 nativo, upload proxy + download presigned, naming de keys, caps 25 MB y TTL 15 min, MIME whitelist, lifecycle Pending | ifran |
| 2026-05-26 | Lifecycle revisado al implementar F4: hard delete coordinado DB → MinIO en vez de soft delete. Sub-decisión Pending de limpieza de huérfanos eliminada (ya no hay backlog por diseño). Convención: soft-vs-hard delete es decisión per-resource, no un default | ifran |
| 2026-05-26 | Estrategia de bucket corregida tras smoke test de F4: `Bun.S3Client` no expone `createBucket` (no es parte de la API), por lo que el bucket DEBE pre-existir. Bootstrap externo: `mc mb` en dev, script CI/CD en prod | ifran |

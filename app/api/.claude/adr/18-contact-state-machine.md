# ADR 18 — Máquina de estados del pipeline de Contact (event-only)

- **Status:** Accepted
- **Fecha de creación:** 2026-05-26
- **Última actualización:** 2026-05-26 (regla de eventos permitidos en `Discarded`)
- **Decisores:** Franco
- **Fase del bootstrap:** actualización (decisión de dominio del módulo `contacts`)
- **ADRs relacionados:** [02-layers-and-dependencies](02-layers-and-dependencies.md), [03-inter-layer-communication](03-inter-layer-communication.md), [13-data-modeling-conventions](13-data-modeling-conventions.md)
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-26 | Decisión inicial: modelo event-only; `AtRisk` agregado; `Discarded` y `Reopened` agregados; eliminación de `changeStateManually` y `stateLocked`; reemplazo de `resolveTargetState` + `isForwardTransition` por `applyTransition`. | Franco |
| 2026-05-26 | Refinamiento: `Discarded` deja de aceptar eventos no-`Reopened`. Se introduce `allowedEventsFor`/`isEventAllowed` en `policies.ts`; `Contact.registerEvent` valida primero la autorización. UI filtra el selector de eventos según el estado. | Franco |

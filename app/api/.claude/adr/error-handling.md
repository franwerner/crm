...
> **Dependencia con `logging.md` (Pending):** esta política es un **contrato vigente y obligatorio desde el momento en que `logging.md` pase a `Accepted`** (cuando se introduzca logging). Mientras esté Pending, la política existe como contrato pero no hay infraestructura que la materialice. No asumir que ya está activa.
...
**Negativas / trade-offs:** la política de logging queda inerte hasta resolver `logging.md`.

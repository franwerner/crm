...
> Status: Pending. Trigger: *cuando un use-case necesite atomicidad sobre más de un repositorio*. En ese momento se introduce un puerto `withTransaction(...)` que el use-case invoca (impl con Drizzle en infra), preservando la regla #2 de `layers-and-dependencies.md` (el use-case no conoce Drizzle).

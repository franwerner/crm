...
Validación de forma y tipos del input crudo en el borde HTTP, con inferencia de tipos TypeScript (un solo schema = validación runtime + tipo estático del input DTO). Integra con Hono vía `@hono/zod-validator`. Cubre la capa perimetral de la decisión "validación en ambos lados" (ver `03-inter-layer-communication.md`); las invariantes de negocio NO van en zod sino en el dominio.
...
- El schema de entrada produce el input DTO tipado que consume el use-case (ver decisión 3.1: entrada DTO, salida view-model).

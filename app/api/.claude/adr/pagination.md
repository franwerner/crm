...
Varios slices necesitan endpoints de listado paginado (contacts, events, state-changes, y cualquier feature futura). Sin un estándar compartido, cada slice inventaría su propia forma de envelope y query params, y `app/ui` (que consume el OpenAPI vía kubb, `api-documentation.md`) se enfrentaría a contratos distintos feature por feature. La paginación es un contrato cross-package — no detalle interno de cada slice.

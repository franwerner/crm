# ADR 13 — Convenciones de modelado de datos

- **Status:** Accepted
- **Fecha de creación:** 2026-05-17
- **Última actualización:** 2026-05-17 (gap UUID v7 resuelto)
- **Decisores:** ifran
- **Fase del bootstrap:** 5.5 (complementa ADR 09)

## Contexto

ADR 09 definió la tecnología de acceso a datos (Drizzle ORM + PostgreSQL, patrón Repository) pero no normó las convenciones de modelado en sí: cómo se definen IDs, qué timestamps lleva cada tabla, cómo se maneja el borrado, cómo se declaran los enums ni las reglas de naming en la capa DB.
...
Este ADR **complementa** ADR 09 (no lo supersede). ADR 09 sigue vigente en todo lo relativo a ORM, migraciones y patrón Repository.
...
- **Naming de la capa DB:** tablas y columnas en `snake_case`. Claves foráneas con sufijo `_id` (ej: `contact_id`, `author_id`, `created_by`). El naming del código TypeScript sigue ADR 11; esta regla aplica solo a la definición del schema Drizzle.
...
- Soft-delete en `Contact`/`User` requiere que todas las queries filtren `deleted_at IS NULL`. Si se olvida, se devuelven registros "borrados". Mitigación: encapsular en el Repository; el use-case nunca escribe la query directamente (ADR 09).
...
| 2026-05-17 | Corregida la regla de PK: decía `DEFAULT gen_random_uuid()` (= v4) y no matcheaba el schema real (`uuid` sin default, id inyectado por la app). Documentado el gap: no hay generador v7 implementado aún | ifran |
| 2026-05-17 | Resuelto el gap de UUID v7: se adopta la lib uuidv7 (ver tech/uuidv7.md) vía generador compartido en src/shared/id; los use-cases deben usar ese generador | ifran |

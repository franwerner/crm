# ADR 14 — Estándar de paginación compartido

- **Status:** Accepted (con sub-decisión cursor-based en Pending)
- **Fecha de creación:** 2026-05-19
- **Última actualización:** 2026-05-19
- **Decisores:** ifran
- **Fase del bootstrap:** 5 (extensión transversal)

## Contexto

Varios slices necesitan endpoints de listado paginado (contacts, events, state-changes, y cualquier feature futura). Sin un estándar compartido, cada slice inventaría su propia forma de envelope y query params, y `app/ui` (que consume el OpenAPI vía kubb, ADR 12) se enfrentaría a contratos distintos feature por feature. La paginación es un contrato cross-package — no detalle interno de cada slice.
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-19 | Decisión inicial. Estándar offset-based con envelope plano (`items`, `total`, `limit`, `offset`); defaults limit 20 / máx 100 / offset 0; interfaces en `shared/types/pagination.ts` y zod en `shared/schemas/pagination.schema.ts`; cursor-based diferido a Pending con trigger | ifran |

...
La API necesita documentación que no se desactualice y que sirva como contrato para el frontend. El frontend (`app/ui`) usa **kubb** para generar tipos/cliente tipados a partir de un spec OpenAPI. Hace falta definir cómo se genera y se expone ese contrato. Conecta con `layers-and-dependencies.md` (capa presentation), `inter-layer-communication.md` §3.1/§3.4 (DTOs de borde + validación zod), `configuration-secrets.md` (config por entorno) y `auth.md`.

## Decisión

**Fuente única de verdad: el schema `zod` del borde.** El mismo schema que valida request/response (`inter-layer-communication.md` §3.4) y define input DTO / output view-model (`inter-layer-communication.md` §3.1) genera el OpenAPI. No se escribe ni mantiene documentación a mano.
...
4. **Contrato = borde, no dominio:** el spec describe los schemas de borde (input DTO / output view-model). Las entidades de dominio NUNCA se exponen (refuerza `inter-layer-communication.md` §3.1).
...
- **Producción:** docs UI **y** endpoint de spec **deshabilitados** vía flag de entorno (gestionado por el módulo de config de `configuration-secrets.md`). No se expone la superficie de la API a Internet.
...
- `@hono/zod-openapi` **define cómo se escribe la capa presentation**: las rutas se declaran con `OpenAPIHono` + `createRoute` (ruta + schema + metadata juntos) en `*.routes.ts`. No es un add-on transparente — impacta el estilo de la capa presentation (ver `layers-and-dependencies.md`).
...
- El endpoint de spec (`/openapi.json`) y la UI Scalar se registran/activan condicionados por el flag de entorno del módulo de config (`configuration-secrets.md`): activos en no-prod, inactivos en prod.

...
**react-hook-form + schemas zod generados por `@kubb/plugin-zod`.** La validación cliente es para UX (feedback inmediato), NO es la fuente de verdad — la API valida (`app/api` `data-access.md` §3.4). Los schemas derivan del mismo OpenAPI: no se escriben a mano. Errores que solo el server conoce ("email ya existe") se muestran desde la respuesta RFC 7807 (ver `error-handling.md`).

...
Decisión cross-paquete tomada en conjunto con `app/api` ADR 10 (que originalmente NO definía el transporte del token — hueco detectado al bootstrapear `app/ui` y resuelto). Topología: `app/api` y `app/ui` se sirven en el **mismo sitio**.
...
- **401 global:** (ADR 04 §4.1) al recibir 401 → limpiar auth-state y redirigir a login.
...
- Token en memoria + re-login al refrescar — peor UX (sin refresh token en `app/api` ADR 10).
...
- Sin refresh token en `app/api` (ADR 10 sub-Pending) → sesiones más cortas / re-login más frecuente. Impacta UX acá; se resuelve del lado de `app/api`.
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-17 | Decisión inicial (cross-paquete con `app/api` ADR 10: cookie httpOnly same-site, front sin manejo de token) | ifran |

# ADR 15 — CRUD UI patterns: inline para escalares, modal para colecciones de subentidades

- **Status:** Accepted
- **Fecha de creación:** 2026-05-26
- **Última actualización:** 2026-05-26
- **Decisores:** ifran
- **Fase del bootstrap:** Update — decisión nueva (relacionada a ADR 11, 13, 14)
...
- `app/ui/src/shared/ui/form-field.tsx` — `FormField` y `InlineFormField` (label + asterisco + description + error consistentes).
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-26 | Decisión inicial. Migración: 12 archivos boilerplate de `projects/components/project-detail/{budget-items,expenses,extensions,responsibles}/` eliminados; `contact-channels-panel.tsx` y `contact-assignees-panel.tsx` migrados de inline a modal | ifran |
| 2026-05-26 | Extensión: `InlineField` generalizado a `shared/ui/`. `FormFieldDescriptor` gana `description?` y `badgeVariants?`. Wrappers `FormField`/`InlineFormField` centralizan label + asterisco rojo + description + error. Project: scalars editables inline (name, contactId, currency, startDate, description) en `ProjectInfoPanel`; `<h1>` y botón "Editar" eliminados; `edit-project-modal.tsx` borrado. `useUpdateProject` ahora acepta `UpdateProjectBody` (partial). Currency lleva description estática siempre visible | ifran |
| 2026-05-26 | Unificación: `description` se rebautiza como `extra` con tipo `string \| ((values) => ReactNode)`. String se renderiza con wrapper estilado; función se evalúa y renderiza sin envolver. InlineField solo consume la forma string. `register-event-modal.tsx` refactorizado a factory `makeRegisterEventForm(pipelineState)` con `extra` función; eliminado el `FieldSlot`. Convención: `FieldSlot` queda solo para reemplazar el control entero | ifran |

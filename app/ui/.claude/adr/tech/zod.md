...
Dos usos: (1) validar `import.meta.env` en el módulo de config al startup (`configuration.md`), fallar rápido si falta una var; (2) validar formularios — los schemas los genera `@kubb/plugin-zod` desde el OpenAPI de `app/api`, conectados a react-hook-form vía `@hookform/resolvers`.

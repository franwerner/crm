# ADR 13 — Styling y Design System (app/ui)

- **Status:** Accepted
- **Fecha de creación:** 2026-05-23
- **Última actualización:** 2026-05-23
- **Decisores:** ifran
- **Fase del bootstrap:** 5.7 (extensión)

## Contexto

`app/ui` no tenía decisión de estilos: ni catálogo de tech para CSS, ni convención de design-system más allá del ADR 11. Existe un handoff de diseño en `docs/visual/handoff/` (`tokens.css` como source of truth + `components-reference.html` como referencia visual) que hay que portar al paquete. Este ADR elige el stack de estilos y cómo se integra con la estructura ya definida (ADR 02 / 11).
...
- Componentes base, genéricos y agnósticos de dominio (los de shadcn + propios reutilizables) → `src/shared/ui/` (plano, ver ADR 11 actualizado).
- Componentes que conocen dominio o que solo usa un feature → `src/features/<f>/components/` (reglas 1/2/6 del ADR 02).
...
- `shadcn add`: el CLI apunta a `src/shared/ui` (alias en `components.json`). Los componentes generados quedan en kebab-case (ADR 11).
- El mapeo primitivos `--ds-*` → roles shadcn vive solo en `globals.css` (capa 2); no duplicar por componente.
- `shared/ui` es presentacional puro (ADR 02 regla 6): los componentes shadcn no se acoplan a API/Query.
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-23 | Decisión inicial | ifran |
| 2026-05-23 | Implementada la base: tokens re-namespaceados a `--ds-*` (3 capas en `globals.css`), shadcn configurado (`components.json`, `cn` en `shared/lib`), Button como smoke test; build/arch/lint en verde | ifran |
| 2026-05-23 | Fuente General Sans vía CDN de Fontshare (`<link>` + preconnect en `index.html`); override ESLint en `shared/ui` para `react-refresh/only-export-components` | ifran |

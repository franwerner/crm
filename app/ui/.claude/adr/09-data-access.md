# ADR 09 — Acceso a datos: kubb + TanStack Query (app/ui)

- **Status:** Accepted
- **Fecha de creación:** 2026-05-17
- **Última actualización:** 2026-05-17
- **Decisores:** ifran
- **Fase del bootstrap:** 5.5 (stack base en Fase 0)

## Contexto

`app/ui` consume `app/api` a través de su OpenAPI (`app/api` ADR 12). El "qué" (kubb + TanStack Query) se decidió en Fase 0; acá se fijan las convenciones de uso y el workflow de regeneración.
...
- **Fuente del spec:** el OpenAPI de `app/api` servido en **dev/staging** (abierto ahí, cerrado en prod — `app/api` ADR 12). kubb NO consume el spec en prod.
...
- **Carpeta generada:** `src/shared/api/` es **artefacto read-only**: no se edita ni se revisa a mano; se regenera. Solo la consumen los hooks de feature (ADR 02 reglas #3 y #5).
- **Estado de servidor:** TanStack Query siempre (ADR 03 §3.2). El `QueryClient` y sus defaults (staleTime, retry) viven en `src/shared/lib`.
...
| Fecha | Cambio | Por |
|---|---|---|
| 2026-05-17 | Decisión inicial (convenciones + workflow de regeneración) | ifran |

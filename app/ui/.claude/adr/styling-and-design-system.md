# ADR — Styling y Design System (app/ui)
...
`app/ui` no tenía decisión de estilos: ni catálogo de tech para CSS, ni convención de design-system más allá del `folder-structure.md`. Existe un handoff de diseño en `docs/visual/handoff/` (`tokens.css` como source of truth + `components-reference.html` como referencia visual) que hay que portar al paquete. Este ADR elige el stack de estilos y cómo se integra con la estructura ya definida (`layers-and-dependencies.md` / `folder-structure.md`).
...
- Componentes base, genéricos y agnósticos de dominio (los de shadcn + propios reutilizables) → `src/shared/ui/` (plano, ver `folder-structure.md` actualizado).
- Componentes que conocen dominio o que solo usa un feature → `src/features/<f>/components/` (reglas 1/2/6 del `layers-and-dependencies.md`).
...
- `shadcn add`: el CLI apunta a `src/shared/ui` (alias en `components.json`). Los componentes generados quedan en kebab-case (`folder-structure.md`).
- El mapeo primitivos `--ds-*` → roles shadcn vive solo en `globals.css` (capa 2); no duplicar por componente.
- `shared/ui` es presentacional puro (`layers-and-dependencies.md` regla 6): los componentes shadcn no se acoplan a API/Query.

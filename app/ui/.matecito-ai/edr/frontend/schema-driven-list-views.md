# EDR — Schema-driven list views (metadata-driven UI)
...
5. **No** se mete presentación (labels, options con label, formato, badge variants) en el backend. Se respeta la separación contrato/presentación de los `../data/data-access.md` y `styling-and-design-system.md`: el backend expone el contrato; la presentación es autónoma del front.
...
- **Presentación en el backend** (labels/options vía OpenAPI `x-*`) — descartado: rompe la separación contrato/presentación (`../data/data-access.md`/`styling-and-design-system.md`).
- **Descriptor no tipado contra el contrato** (objeto manual independiente) — descartado: vuelve a abrir la puerta a la desincronización silenciosa que este EDR busca cerrar.
- **Status quo** (definiciones manuales duplicadas por entidad) — descartado: es el problema que motiva este EDR.
...
- **Descriptor por entidad** (sabe de dominio: labels en español, options con label, badge variants, formato) en `features/<f>/<f>.descriptor.ts`. Por `../structure/folder-structure.md`, lo que "sabe" de dominio va en `features/`, no en `shared/`.

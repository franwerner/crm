# EDR — Acceso a datos: kubb + TanStack Query (app/ui)
...
`app/ui` consume `app/api` a través de su OpenAPI (`app/api` `api-contract.md`). El "qué" (kubb + TanStack Query) se decidió en Fase 0; acá se fijan las convenciones de uso y el workflow de regeneración.
...
- **Fuente del spec:** el OpenAPI de `app/api` servido en **dev/staging** (abierto ahí, cerrado en prod — `app/api` `api-contract.md`). kubb NO consume el spec en prod.
...
- **Carpeta generada:** `src/shared/api/` es **artefacto read-only**: no se edita ni se revisa a mano; se regenera. Solo la consumen los hooks de feature (`../structure/layers-and-dependencies.md` reglas #3 y #5).
- **Estado de servidor:** TanStack Query siempre (`../structure/inter-layer-communication.md` §3.2). El `QueryClient` y sus defaults (staleTime, retry) viven en `src/shared/lib`.

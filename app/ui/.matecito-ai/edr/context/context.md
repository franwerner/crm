# EDR — Contexto del proyecto (app/ui)
...
- **Consumo de API:** kubb genera tipos + cliente + hooks de react-query desde el OpenAPI de `app/api` (`api-contract.md` de la API).
- **Equipo:** 1 (solo dev).
- **Madurez:** greenfield.
...
**Negativas / trade-offs:** dependencia fuerte del contrato OpenAPI de `app/api` (mitigada con regeneración en CI — `../data/data-access.md`).

## Reglas concretas

- Todo EDR de este paquete vive en `app/ui/.matecito-ai/edr/`. Autónomo respecto de `app/api`.
